...
class RosBridge:
    ...
    def __init__(self, real_robot=False):
        ...
        # Rviz Path publisher
        self.mir_exec_path = rospy.Publisher('husky_exec_path', Path, queue_size=10)
    ...
    def set_state(self, state_msg):
        ...
        if not self.real_robot :
            ...
            self.set_model_state('/', copy.deepcopy(state[3:6]))
    ...
    def callbackState(self,data):
        ...
        if self.reset.isSet():
            ...
            self.mir_path.poses.append(stamped_husky_pose)
            self.mir_exec_path.publish(self.husky_path)
    ...
    def LaserScan_callback(self,data):
        if self.get_state_event.isSet():
            scan = data.ranges
            scan = scan[8:len(scan)-8] # remove first 8 and last 8 elements from laser scan because they are 0.0 in scan on real MiR100
